...
class cproduct extends CI_Controller{
	...
	public function savestyling()
	{
		$this->chkloginuser();
		$this->load->model('mstyling');
		...
		$oper=$this->input->post('oper');
		$id=$this->input->post('id');
		$res=$this->mstyling->savestyling($productstore);
		echo json_encode($res);
	}
	...
	public function lookupStyling()
	{
		$this->chkloginuser();
		$numberofrecords=15;
		$this->load->model('mlogin');
		$Cnt=$this->mlogin->getTableCount('styling');
		$this->load->view('vlookupstyling',array('Cnt'=>$Cnt,'numberofrecords'=>$numberofrecords));
	...
	public function loadStyling()
	{
		...
		$start=$end-$numberofrecords;
		$this->load->model('mstyling');
		$res=$this->mstyling->loadStyling($start,$numberofrecords);
		//print_r($res); die();
		echo json_encode($res);
	}
	...
	public function deletestyling($id='')
	{
		...
		$id=$this->input->post('id');
		$this->load->model('mstyling');
		$res=$this->mstyling->deleteStyling($id);
		echo json_encode($res);
	}
}
?>