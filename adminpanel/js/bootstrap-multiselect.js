...
!function($) {
    ...
    Multiselect.prototype = {
                            ...
                            if (this.query !== event.target.value) {
                                ...
                                $.each($('li', this.$ul), $.proxy(function(index, element) {
                                    ...
                                    if (value !== this.options.selectAllValue && text) {
										...
										else if(value == undefined || value == 'on')// added so that optgroups are visible in all conditions
											showElement = true;
        ...
        select: function(selectValues) {
            ...
            for (var i = 0; i < selectValues.length; i++) {
                ...
                if (this.options.selectedClass) {
					...
					//console.log(this.options.selectedClass);
		...
		/*Code Added by Rachana */
		getselect: function(selectValues) {
			if(selectValues && !$.isArray(selectValues)) {
				selectValues = [selectValues];
			}
			...
			for (var i = 0; i < selectValues.length; i++) {
				...
				var $checkbox = this.getInputByValue(value);
					...
					//console.log('option:'+this.getOptionByValue(value));
					console.log('selectValues:'+selectValues);
					//$(".multiselect-container li").removeClass('active');
					var textpart=$(".multiselect form-control option[value='"+value+"']").text();
					console.log('textpart:'+textpart);
					//$(".multiselect-container li input").find("[value=" + value + "]").parents().eq(2).addClass('active');
					//console.log('in value:'+value);
						//$(".multiselect-container li input").find("[value=" + value + "]").parents().eq(2).addClass(this.options.selectedClass);
						//$(".btn-group button").html(textpart);
					/*if($(".multiselect-container li input").find("[value=" + value + "]"))
					{
						alert('in find:'+value);
						$(".multiselect-container li").addClass('active');
						$(".multiselect-container li").addClass(this.options.selectedClass);
					}*/
				   /* if (this.options.selectedClass) {
						console.log('v1:'+this.options.selectedClass);
					   // $checkbox.parents('li').addClass(this.options.selectedClass);
					   $(".multiselect-container li input").removeClass(this.options.selectedClass);
					   $(".multiselect-container li input").find("[value=" + value + "]").addClass('active');
					}*/
        ...
        dataprovider: function(dataprovider) {
            ...
            var optionDOM = "";
			optionDOM += '<option value="">Select</option>';
			if(dataprovider != null)
			{
			var type='';
			dataprovider.forEach(function (option) {
				/* Code Added for grouping by Akshata */
				if(option.group=='yes')
				{
					if(type !== option.type)
					{
						optionDOM += '<optgroup label='+ option.type +' ></option>';
						type=option.type;
					}
				}
